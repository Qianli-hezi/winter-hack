...
def determine_frontier_path(state: SimulationState) -> None:
    ...
    if state.frontier_goal == state.goal["cell"]:
        ...
        #
        # Frontier selection logic (goal-directed improved version)
        #
        if not frontiers:
            print("No frontiers detected → fallback to final goal.")
            ...
            dist_to_goal = math.hypot(gx - robot_x, gy - robot_y)

            # Adjust weights dynamically based on proximity to goal
            if dist_to_goal > 1.0:
                # far → explore more
                w_len, w_goal, w_head, w_bfs = 1.0, 1.2, 0.4, 0.3
            else:
                # near → strongly bias toward goal
                w_len, w_goal, w_head, w_bfs = 0.7, 2.0, 0.2, 0.1

            scores = {}
            for fcell in frontiers:
                fx, fy = cell_center(fcell, cell_size)

                # try path planning to estimate cost
                start_cell = pose_to_cell(state.world, state.pose)
                try:
                    path = plan_unknown_world(state, start_cell, fcell)
                except Exception:
                    path = []
                if not path:
                    continue
                ...
                d_goal = math.hypot(fx - gx, fy - gy)
                # “goal proximity bonus”: smaller d_goal → larger bonus
                goal_bonus = 1.0 / (1.0 + d_goal)

                # heading alignment
                dx = fx - robot_x
                dy = fy - robot_y
                frontier_angle = math.atan2(dy, dx)
                heading_diff = abs(ang_diff(frontier_angle, robot_theta))

                bfs_dist = state.frontier_distances.get(fcell, d_goal / cell_size)
                ...
                score = (
                    ...
                    + w_head * heading_diff
                    + w_bfs * bfs_dist
                    - w_goal * goal_bonus  # reward close-to-goal candidates
                )
                scores[fcell] = score

            if scores:
                best_frontier_cell = min(scores, key=scores.get)
                fx, fy = cell_center(best_frontier_cell, cell_size)
                print(
                    f"Selected frontier {best_frontier_cell} "
                    f"→ score={scores[best_frontier_cell]:.3f}, "
                    f"goal_dist={math.hypot(fx-gx, fy-gy):.2f}m"
                )
            else:
                # fallback if all paths failed
                print("No reachable frontiers via planner; using closest to goal.")
                best_frontier_cell = min(
                    frontiers,
                    key=lambda c: math.hypot(
                        cell_center(c, cell_size)[0] - gx,
                        cell_center(c, cell_size)[1] - gy,
                    ),
                )
